...
static const char *TAG = "USER_esp_https_ota";
...
static esp_err_t _http_handle_response_code(esp_http_client_handle_t http_client, int status_code)
{
    ...
    if (status_code == HttpStatus_MovedPermanently || status_code == HttpStatus_Found || status_code == HttpStatus_TemporaryRedirect) {
        ...
        if (err != ESP_OK) {
            ...
            extern void ota_send_OTA_FAIL_URL_RE_ERR(void);
            ota_send_OTA_FAIL_URL_RE_ERR();
            return err;
        }
        ...
        extern void ota_send_OTA_FAIL_URL_ERR(void);
        ota_send_OTA_FAIL_URL_ERR();
        return ESP_FAIL;
...
static esp_err_t user_esp_https_ota_begin(esp_https_ota_config_t *ota_config, esp_https_ota_handle_t *handle)
{
    esp_err_t err;
...
esp_err_t user_esp_https_ota(const esp_http_client_config_t *config)
{
    ...
    esp_err_t err = user_esp_https_ota_begin(&ota_config, &https_ota_handle);
